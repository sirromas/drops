...
class Feedback extends Utils
{
    ...
    {
        $query
                = "select * from mdl_context where id=$contextid and contextlevel=50";
    ...
    function get_student_courses($userid)
    {
        ...
        $query
              = "select * from  mdl_role_assignments where roleid=5 and userid=$userid";
    ...
    function add_student_suggest($item)
    {
        $now = time();
        $query
             = "insert into mdl_suggest (courseid,userid,type,msg,added) 
    ...
    function add_student_feedback($item)
    {
        ...
        $msg            = $userdata->text;
        $query
                        = "insert into mdl_contact_requests (name,email,phone,msg,added) 
    ...
    function upload_resume($post, $file)
    {
        ...
        if ($file_data['error'] == 0 && $file_data['size'] > 0) {
            ...
            $upload_path = $_SERVER['DOCUMENT_ROOT']
                . "/lms/custom/feedback/resume/$filename";
            if (move_uploaded_file($file_data['tmp_name'], $upload_path)) {
                $now = time();
                $query
                     = "insert into mdl_students_resume (userid, path, added)  
    ...
    function get_manager_feedback($type)
    {
        $list    = "";
        $susers  = array();
        $userid  = $this->user->id;
        ...
        if (count($courses) > 0) {
            $cs  = implode(',', $courses);
            $query
                 = "select * from mdl_suggest where courseid in ($cs) and type=$type";
            $num = $this->db->numrows($query);
            if ($num > 0) {
                $result = $this->db->query($query);
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        ...
        $list .= $this->create_manager_feedback_table($susers);

        return $list;
    }

    function create_manager_feedback_table($items)
    {
        $list = "";
        ...
        if (count($items) > 0) {
            foreach ($items as $item) {
                ...
                $link       = "<a href='https://" . $_SERVER['SERVER_NAME']
                    . "/lms/user/profile.php?id=$item->userid' target='_blank'>$name</a>";
