...
class Enroll
{
    ...
    {
        $query
            = "insert into mdl_user (confirmed, mnethostid, username, password) values (1, 1, '$user->email', '$pwd')";
        $this->db->query($query);
     ...
     * @param $courseid
     */
    function assign_roles($userid, $courseid, $role = 0)
    {
        $roleid           = ($role == 0) ? self::STUDENT_ROLE : $role;
    ...
    function add_paypal_payment($user)
    {
        $now = time();
        $query
             = "insert into mdl_paypal_payments 
                (courseid,
                 userid,
                 psum,
                 trans_id,
     ...
     * @return int
     */
    function is_manager_already_enrolled($courseid, $userid)
    {
        $mrole     = self::MANAGER_ROLE;
        $contextid = $this->getCourseContext($courseid);
        $query
                   = "select * from mdl_role_assignments 
                    where contextid=$contextid and 
